...
// PID
const float KP = 0.139; // 0.15 || 0.139
const float KI = 0.00135; // 0.0015 || 0.00135
const float KD = 0.86; // 0.9 || 0.86
const float KV = 7; // 6.5 || ''
...
// Giro cerrado a la izquierda (90°)
void forceIzquierda(){
  motores(-fvmax, fvmax);   // rueda izquierda atrás, derecha adelante
  delay(300);               // ajustar según pista
  while(digital[3]==0 && digital[4]==0){ // hasta volver a detectar la línea central
    lectura();
    motores(-fvmax, fvmax);
  }
}

// Giro cerrado a la derecha (90°)
void forceDerecha(){
  motores(fvmax, -fvmax);   // rueda derecha atrás, izquierda adelante
  delay(300);               // ajustar según pista
  while(digital[3]==0 && digital[4]==0){ // hasta volver a detectar la línea central
    lectura();
    motores(fvmax, -fvmax);
