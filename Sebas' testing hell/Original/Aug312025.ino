...
// PID 
const float KP = 0.144; // 0.15 || 0.139
const float KI = 0.00195; // 0.0015 || 0.00135
const float KD = 1; // 0.9 || 0.86
const float KV = 10; // 6.5 || ''
...
int readSensors(void){
  ...
  if (sensorSum == 0){position = -1;} else{position = (weightSum / sensorSum);}

  if(lastPosition <= 100 && position == -1){
    position = 0;
  }
  if(lastPosition >= 600 && position == -1){
    position = 700;
  }
...
void getPID() {
  ...
  motorSpeed(vbase+DF+leftTrim, vbase-DF+rightTrim);
}
...
void holdBrakes(){
  if (position <= 100){
    digitalWrite(STBY,HIGH);
    motorSpeed(fvmax, -fvmin);
  }
  else if (position >= 600){
    digitalWrite(STBY,HIGH);
    motorSpeed(-fvmin, fvmax);
  }
}
...
bool intersectLines(){
 ...
 // Check for a sharp 90-degree turn to the right
 if (digital[7] == 1 && sensorSum < 3) {
    moveRight();
 }
  // Check for a sharp 90-degree turn to the left
 else if (digital[0] == 1 && sensorSum < 3) {
  moveLeft();
 }
  // Default behavior for wide intersections or dead ends
  else {
    moveForward();
 }
 digitalWrite(LED, 0);
