...
const unsigned long changeDelay = 3000; // Duración antes de cambiar los valores
int vmin = 50; // Velocidad minima
int vmax = 90; // Velocidad máxima
// Resto de la carrera
int vxmin = 100;
int vxmax = 140;
int fadelante = 200; // Frenos hacia delante
...
int lectura(void){
  ...
  if(lastpost <= 50 && post == -1){
    post = 0;
  }
  if(lastpost >= 650 && post == -1){
    post = 700;
  }
...
void PID(){
  ...
  if((proporcional*integral)<0) integral = 0;{
    ...
    motores(vbase + diferencial + set0, vbase - diferencial + set1);
  }
}
